...
latent = torch.load('pe_feature.pt')
...
# 重塑为16x16的网格
heatmap = heatmap.reshape(32, 32).detach().cpu().numpy()

# # 可视化
plt.figure(figsize=(10, 10))
plt.imshow(heatmap, cmap='viridis')  # 使用与示例图相似的蓝绿色调
plt.colorbar()
plt.title("pe Feature Activation (Channel Mean)")
plt.axis('off')
plt.savefig('pe_vis.png')

# from sklearn.decomposition import PCA
# features = latent
# # 展平特征并应用PCA
# flatten_features = features.squeeze().detach().cpu().numpy()
...
# rgb_map = normalized.reshape(32, 32, 3)
# plt.figure(figsize=(6, 6))
# plt.imshow(rgb_map)
# plt.axis('off')
# plt.title('pe Patch Feature PCA RGB Map')
# plt.savefig('pe_pca_rgb_map.png')
# plt.show()