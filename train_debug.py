...
def print_gpu_memory_usage(message, device='cuda'):
    """打印当前 GPU 显存使用情况"""
    allocated = torch.cuda.memory_allocated(device) / 1024**3
    reserved = torch.cuda.memory_reserved(device) / 1024**3
    print(f"######{message} - GPU Memory Allocated: {allocated:.3f} GB, Reserved: {reserved:.3f} GB")
