...
def get_orbit_camera_location(
    ...
    if not (num_elevation_rings == 1 and only_northern_hemisphere):
...
def deep_cleanup():
    ...
    # if bpy.app.version >= (3, 0, 0):
        ...
        # bpy.ops.wm.tool_set_by_id(name="builtin.debug")
        # bpy.ops.wm.debug_menu(op='MEMORY')
...
def render_object(
    ...
    for i in range(num_renders):
        ...
        # np.save(os.path.join(output_dir, f"{i:03d}-location.npy"), location)
...
if __name__ == "__main__":
    ...
    logger = init_logging('render_log')
    ...
    # obj_paths = [os.path.join(args.object_path, filename) for filename in os.listdir(args.object_path)]
    obj_paths = [args.object_path]
    for idx, obj_path in enumerate(obj_paths):
        save_uid = obj_path.split('/')[-1].split('.')[0]
        # get the target directory for the rendering job
        if len(args.output_dir.split('/')[-1]) == 32:
            target_directory = args.output_dir
        else:
            target_directory = os.path.join(args.output_dir, save_uid)
        os.makedirs(target_directory, exist_ok=True)
        logger.info(f'{idx}/{len(obj_paths)}, proccessing {obj_path[83:]}')
        
        contents = os.listdir(target_directory)
        if len(contents) == 65:
            logger.info(f'{save_uid} is rendered, skip it')
            continue
        ...
        if (
            (len(png_files) != args.num_renders)
            or (len(npy_files) != args.num_renders)
            or (len(metadata_files) != 1)
        ):
            logger.error(f"{save_uid} occures error")
